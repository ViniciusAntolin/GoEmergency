...
public class ExcluirEndereco extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        int idEndereco = Integer.parseInt(request.getParameter("idenderecoexcluir"));
        try{
            GenericDAO dao = new EnderecoDAOImpl();//Add importação
            dao.excluir(idEndereco);
            request.getRequestDispatcher("ListarEndereco").forward(request, response);            
        }catch(Exception ex){
            System.out.println("Erro no Servlet ExcluirEndereco: "+ex.getMessage());
            ex.printStackTrace();
        }
    }
